...
_AnyManifold = (
    "Sphere | SphereN | Euclidean | Torus | SE2 | SO2 | SO3 | SE3 | "
    "ConfigurationSpace | Product"
)
_AnyMetric = (
...
class SE3:
    """The special Euclidean group SE(3) = R³ ⋊ SO(3).
...
class Product:
    """The Riemannian product of several manifolds, M₁ × M₂ × ⋯ × Mₖ.

    Points and tangents are the block-concatenation of the sub-manifold points and
    tangents; exp/log/geodesic act block-wise and distance is the L2 combination of
    the block distances. Example: ``Product([Euclidean(3), SE2()])`` is an R³ × SE(2)
    mobile-manipulator configuration space.
    """

    def __init__(self, manifolds: list[_AnyManifold]) -> None: ...
    def dim(self) -> int: ...
    def random_point(self) -> _Point: ...
    def inner(self, p: _Point, u: _Tangent, v: _Tangent) -> float: ...
    def norm(self, p: _Point, v: _Tangent) -> float: ...
...
class InterpolationResult:
    ...
    def path(self) -> np.ndarray:
        """(N, d) float64 ndarray of points traced from start toward target (starts with ``start``)."""
        ...

    @property
    def waypoints(self) -> list[_Point]:
        """The path as a Python ``list`` of point arrays (back-compat with the old ``path`` API)."""
        ...
